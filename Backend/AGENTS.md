...
이 파일은 향후 Codex 세션이 이 프로젝트의 맥락을 빠르게 이해하도록 돕는 문서입니다.
...
GET  /oauth2/authorization/google — 구글 로그인 시작 (→ 구글 리다이렉트)
     → 성공 시 {frontend}/oauth/callback?token={JWT} 로 리다이렉트
```
...
→ OAuth2SuccessHandler: Member 저장 + JWT 발급
→ {frontend}/oauth/callback?token=eyJhbG... 으로 리다이렉트
...
- **출처 표기(`dataSources`)** — 단계별 사용 데이터 출처 명시 (KOTRA, 무역보험공사, yesTrade, Codex AI)
- **AI 프롬프트에 안전 원칙 내장** — "안내 vs 판정" 구분, 교차 해석, 데이터 지어내지 않기, 모르면 모른다
- **전략물자**: 절대 "수출 가능" 단정 금지 → "통제 가능성 있음 → yesTrade 공식 자가판정 필수"로 안내

## 외부 API 목업 폴백

모든 외부 서비스(Codex, KOTRA, K-SURE, yesTrade)는 **API 키 미설정 시 현실적인 목업 데이터 반환**:
...
- `${CLAUDE_API_KEY:}` — Codex API 키 (없으면 목업)
...
```
/api/*           → http://localhost:8081  (백엔드 REST API)
/oauth2/*        → http://localhost:8081  (Google OAuth 시작)
/login/oauth2/*  → http://localhost:8081  (OAuth 콜백 처리)
```
...
- [ ] Phase 9: Codex API 키 연동 (실제 LLM 호출)
